...
export default function ContactForm({ accessKey }: Props) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!accessKey) {
      setStatus('error');
      setErrorMsg('El formulario no está configurado. Escríbenos por WhatsApp.');
      return;
    }
    ...
    try {
      ...
      setStatus('error');
      setErrorMsg('No pudimos enviar el formulario. Escríbenos por WhatsApp.');
    } catch {
      setStatus('error');
      setErrorMsg('No hay conexión con el correo. Escríbenos por WhatsApp.');
    }
  };
